...
function HostCreateGame() {
  ...
  function addQuestion() {
    ...
    const newQuestion = { text: questionText, answers: formattedAnswers };
    ...
    <div className="min-h-screen bg-gray-950 text-white font-sans">
      ...
      <div className="border-b border-white/10 px-8 py-4 flex items-center justify-between bg-gray-900/60 backdrop-blur-sm sticky top-0 z-10">
        ...
        <div className="flex gap-3">
          <button
            onClick={saveQuiz}
            className="px-4 py-2 text-sm font-medium rounded-lg border border-white/20 text-white/70 hover:bg-white/10 hover:text-white transition-all duration-150"
          >
            Save Draft
          </button>
          <button
                  ...
                  {q.answers.map((a, j) => (
                    <span
                      ...
                      className={`text-xs px-1.5 py-0.5 rounded font-medium ${
                        a.correct ? "bg-emerald-500/20 text-emerald-400" : "bg-white/5 text-white/30"
                      }`}
                    >
                      {answerLabels[j]}
                    </span>
            ...
            {questions.length > 0 && (
              <div className="mt-6 flex gap-4 p-4 bg-white/5 rounded-xl border border-white/10 text-sm text-white/50">
                ...
                <span>✅ <strong className="text-white">
                  {questions.reduce((acc, q) => acc + q.answers.filter(a => a.correct).length, 0)}
